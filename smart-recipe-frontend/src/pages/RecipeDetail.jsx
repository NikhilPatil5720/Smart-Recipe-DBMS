...
//new version  functional and styled
...
export default function RecipeDetail() {
  ...
  const [editingText, setEditingText] = useState(""); // Text for comment being edited
  ...
  const handleSaveEdit = async (comment_id) => {
    ...
    try {
      const token = localStorage.getItem("token");
      setAuthToken(token);
      await API.put(`/comments/${comment_id}`, { comment_text: editingText });
      const res = await API.get(`/comments/${id}`);
      setComments(res.data);
            ...
            <ul className="space-y-4 mb-6">
              {comments.map((c, idx) => (
                <li key={idx} className="bg-gray-50 p-4 rounded-xl shadow-sm">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-4 py-2 bg-blue-500 text-white rounded-xl shadow hover:bg-blue-600 transition transform hover:-translate-y-0.5 hover:shadow-lg"
                        >
                          Save
                        </button>
                        <button
                          onClick={() => setEditingCommentId(null)}
                          className="px-4 py-2 bg-gray-300 text-gray-700 rounded-xl shadow hover:bg-gray-400 transition transform hover:-translate-y-0.5 hover:shadow-lg"
                        >
                          Cancel
                        </button>
                      </div>
                  ...
                  {currentUser && currentUser.user_id === c.user_id && editingCommentId !== c.comment_id && (
                    <div className="flex gap-3 mt-2">
                      <button
                        onClick={() => { setEditingCommentId(c.comment_id); setEditingText(c.comment_text); }}
                        className="px-4 py-2 bg-blue-500 text-white rounded-xl shadow hover:bg-blue-600 transition transform hover:-translate-y-0.5 hover:shadow-lg"
                      >
                        Edit
                      </button>
                      <button
                        ...
                        className="px-4 py-2 bg-red-500 text-white rounded-xl shadow hover:bg-red-600 transition transform hover:-translate-y-0.5 hover:shadow-lg"
                      >
                        Delete
                      </button>
                    </div>
